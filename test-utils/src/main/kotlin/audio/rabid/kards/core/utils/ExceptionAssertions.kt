...
class ExpectThrow<T>(
    ...
    val to: ExpectThrow<T>
        get() {
            words.add("to")
            return this
        }

    val not: ExpectThrow<T>
        get() = this.apply {
            words.add("not")
            negative = !negative
        }
        ...
        val res = try {
            ...
            if (negative || !clazz.isAssignableFrom(e.javaClass)) {
                throw AssertionError(
                    "Expected block ${words.joinToString(" ")} ${clazz.simpleName} but it threw ${e.javaClass.simpleName}",
                    e
                )
            }
            return
    ...
    inline fun <reified E : Throwable> `throw`(noinline exceptionBlock: (E) -> Unit) =
        `throw`(E::class.java, exceptionBlock)
}
