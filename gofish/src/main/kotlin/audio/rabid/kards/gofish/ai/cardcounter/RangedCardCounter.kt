...
class RangedCardCounter(players: List<PlayerName>) : ArrayHelper<PlayerName, Rank> {
...
fun RangedCardCounter.setNone(playerName: PlayerName, rank: Rank) = setExactly(playerName, rank, 0)

fun RangedCardCounter.setAtLeastOne(playerName: PlayerName, rank: Rank) {
    if (getMin(playerName, rank) == 0) setMin(playerName, rank, 1)
}

fun RangedCardCounter.addExactly(playerName: PlayerName, rank: Rank, count: Int) {
    setMin(playerName, rank, getMin(playerName, rank) + count)
    setMax(playerName, rank, getMax(playerName, rank) + count)
}

fun RangedCardCounter.setExactly(playerName: PlayerName, rank: Rank, count: Int) {
    setMin(playerName, rank, count)
    setMax(playerName, rank, count)
}

fun RangedCardCounter.addPossiblyOneMore(playerName: PlayerName, rank: Rank) =
    setMax(playerName, rank, getMax(playerName, rank) + 1)

fun RangedCardCounter.getMinMax(playerName: PlayerName, rank: Rank): Pair<Int, Int> =
    getMin(playerName, rank) to getMax(playerName, rank)

fun RangedCardCounter.getDifferential(playerName: PlayerName, rank: Rank) =
