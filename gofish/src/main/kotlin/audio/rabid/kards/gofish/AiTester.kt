...
class AiTester(
    ...
    private val cpuMaxingContext = newFixedThreadPoolContext(Runtime.getRuntime().availableProcessors() + 3, "CPU")
    ...
    fun run() = runBlocking {
        ...
        repeat(testRuns) { i ->
            ...
            jobs.add(launch(cpuMaxingContext) {
                ...
                val game = GoFishGame(
                        playerInfo = mapOf(
                                PlayerName("smart") to aiUnderTest,
                                *(0 until otherPlayers).map { i ->
                                    PlayerName("dummy$i") to DumbAi(random)
                                }.toTypedArray()
                        ),
                        random = random
                )
        ...
        println("wins: ${wins.get()} (${wins.get() / testRuns.toDouble()})\t" +
